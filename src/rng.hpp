...
template <typename T> struct rng_state_t {
  typedef T real_t;
  static HOSTDEVICE size_t size() { return 4; }
  uint32_t state[4];
  HOSTDEVICE uint32_t &operator[](size_t i) { return state[i]; }
};
...
template <typename T> inline void xoshiro_jump(rng_state_t<T> &state) {
...
template <typename T> inline void xoshiro_long_jump(rng_state_t<T> &state) {
...
template <> inline HOSTDEVICE double unif_rand(rng_state_t<double> &state) {
  const uint32_t value = xoshiro_next(state);
  double rand =
      value * CURAND_2POW32_INV_DOUBLE + (CURAND_2POW32_INV_DOUBLE / 2.0);
  return rand;
}

template <> inline HOSTDEVICE float unif_rand(rng_state_t<float> &state) {
  const uint32_t value = xoshiro_next(state);
...
template <typename T> class pRNG {
public:
  ...
  rng_state_t<T> &state(size_t i) { return state_[i]; }
...
#ifdef __NVCC__
template <typename T>
DEVICE rng_state_t<T>
get_rng_state(const interleaved<uint32_t> &full_rng_state) {
